...
const API_BASE_URL = (process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000').replace(/\/$/, '').replace(/\/api\/v1$/, '');
...
export default function DashboardPage() {
  ...
  const loadDashboardData = useCallback(async (silent = false) => {
    ...
    try {
      const [analyticsRes, confRes, passRes, inspRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/v1/telemetry/analytics`, { headers, cache: 'no-store' }),
        fetch(`${API_BASE_URL}/api/v1/analytics/confidence-distribution`, { headers, cache: 'no-store' }),
        fetch(`${API_BASE_URL}/api/v1/analytics/pass-rate-benchmark`, { headers, cache: 'no-store' }),
        fetch(`${API_BASE_URL}/api/v1/telemetry/inspections`, { headers, cache: 'no-store' }),
      ]);
      ...
      if (!analyticsRes.ok || !inspRes.ok) throw new Error('Live telemetry is unavailable.');
      ...
      if (!silent) setError('Telemetry stream is offline. Showing the most recently received data.');
    } finally {
  ...
  const handleRunScan = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/api/v1/telemetry/inspections/scan`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        const inspection: InspectionItem = {
          ...
          defects_found: newScan.defects_detected,
          max_severity: newScan.severity,
          severity_score: newScan.severity === 'HIGH' ? 75 : 0,
          confidence: newScan.confidence,
          defect_types: newScan.defects_detected ? ['Simulated Defect'] : [],
          overall_status: newScan.overall_status,
        };
        setInspections((prev) => [inspection, ...prev.filter((item) => item.id !== inspection.id)]);
        updateChartsRealtime(newScan.overall_status, newScan.confidence);
        setConnectionStatus('live');
        setLastUpdated(new Date());
        window.setTimeout(() => void loadDashboardData(true), 500);
      } else {
        throw new Error(`Live scan failed (${res.status})`);
      }
    } catch (err) {
      setConnectionStatus('offline');
      setError(err instanceof Error ? err.message : 'Unable to execute the live scan.');
    } finally {
  ...
  const handleProcessImageInference = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/inspection/analyze-image`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const downloadProductionReport = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/inspection/report-pdf`, {
        method: 'POST',
  ...
  return (
    <div className="studio-dashboard flex min-h-screen bg-[#0B0F19] text-slate-100 font-sans">
      {sidebarOpen && <button aria-label="Close navigation" className="fixed inset-0 z-30 bg-slate-950/70 backdrop-blur-sm lg:hidden" onClick={() => setSidebarOpen(false)} />}
      {/* SIDEBAR NAVIGATION */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-slate-800/80 bg-[#0F172A]/95 p-4 flex flex-col justify-between shrink-0 backdrop-blur-xl transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button aria-label="Close menu" onClick={() => setSidebarOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-white lg:hidden"><X size={18} /></button>
          </div>
      ...
      {/* MAIN CONTENT AREA */}
      <main className="min-w-0 flex-1 p-4 sm:p-6 overflow-y-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800/60 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <button aria-label="Open navigation" onClick={() => setSidebarOpen(true)} className="rounded-lg border border-slate-800 bg-slate-900 p-2 text-slate-300 lg:hidden"><Menu size={18} /></button>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className={`rounded-xl border px-3 py-1.5 text-xs font-semibold flex items-center gap-2 ${connectionStatus === 'live' ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400' : connectionStatus === 'offline' ? 'border-red-500/20 bg-red-500/10 text-red-400' : 'border-amber-500/20 bg-amber-500/10 text-amber-400'}`}>
              {connectionStatus === 'live' ? <Wifi size={14} /> : <WifiOff size={14} />}
              {connectionStatus === 'live' ? `Live · ${lastUpdated?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) || 'syncing'}` : connectionStatus === 'offline' ? 'Offline' : 'Connecting'}
            </div>
            <button aria-label="Refresh live data" title="Refresh live data" onClick={() => void loadDashboardData()} disabled={isRefreshing} className="rounded-xl border border-slate-800 bg-slate-900/80 p-2 text-slate-400 hover:border-blue-500/40 hover:text-blue-400 disabled:opacity-50 transition">
            ...
            <div className="hidden xl:flex rounded-xl border border-slate-800 bg-slate-900/80 px-3 py-1.5 text-xs text-slate-400 items-center gap-1.5">
              <span>📅</span> {new Intl.DateTimeFormat(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }).format(new Date())}
            </div>
        ...
        <section className="studio-banner">
          <div><p className="scene-kicker">YOUR QUALITY CONTROL WORKSPACE</p>
            <h3>Precision starts with a closer look.</h3>
            <p>Inspect every detail, understand the defects, and turn your findings into confident production decisions.</p>
          </div>
          <div className="banner-emblem" aria-hidden="true"><ScanLine size={36} strokeWidth={1.2} /></div>
        </section>
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => window.open(`${API_BASE_URL}/api/v1/telemetry/reports/export-csv`, '_blank')}
        ...
        {userRole === 'SUPERVISOR' && activeTab === 'Defect Trends' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl border border-slate-800 bg-[#0F172A]/80 p-5 space-y-4">
              <h3 className="text-sm font-bold text-white">Recent Defect Frequency</h3>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={liveTrendData}
                  >
                    <XAxis dataKey="scan" stroke="#64748b" fontSize={11} />
